...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  try {
    ...
    if (pointsPerDollar > 0 && rewardValuePerPoint > 0) {
      return {settings: {pointsPerDollar, rewardValuePerPoint }};
    }}
    catch(_error){
...
export const action = async ({request}: ActionFunctionArgs)=>{
      ...
      if(!Number.isFinite(pointsPerDollar) || pointsPerDollar <= 0){
        errors.pointsPerDollar = "Points per dollar must be greater than zero";
      }
      if(!Number.isFinite(rewardValuePerPoint) || rewardValuePerPoint <= 0){
        errors.rewardValuePerPoint = "Reward value per point must be a positive number";
      }
        ...
        metafields: [
          {
            ...
            key: "settings",
            type: "json",
            value: JSON.stringify({pointsPerDollar, rewardValuePerPoint}),
          },
        ],
      },
    },
...
export default function Index() {
  ...
  const [rewardValuePerPoint, setRewardValuePerPoint] = useState(String(settings.rewardValuePerPoint));
  const isSaving =
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      if (fetcher.data?.ok) {
        shopify.toast.show("Loyalty settings saved successfully");
